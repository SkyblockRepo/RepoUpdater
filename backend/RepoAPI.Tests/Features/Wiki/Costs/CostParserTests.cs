...
public class CostParserTests
{
	...
	public void CostParser_CoinsParsesCorrectly()
	{
		...
		var cost = ParserUtils.ParseUpgradeCost(input);
	...
	public async Task CostParser_ItemsParseCorrectly()
	{
		...
		var logger = Substitute.For<ILogger<SkyblockRepo.SkyblockRepo>>();
		var repo = new SkyblockRepo.SkyblockRepo(new SkyblockRepoConfiguration
		{
		...
		var cost2 = ParserUtils.ParseUpgradeCost(input2);
		...
		var cost3 = ParserUtils.ParseUpgradeCost(input3);
		...
		var cost4 = ParserUtils.ParseUpgradeCost(input4);
		...
		var cost5 = ParserUtils.ParseUpgradeCost(input5);
